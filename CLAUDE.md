...
**回收落点与最小释放量是同一算术的两面**（Story 71.2 AC1+AC2）：`landingPct = threshold / compactHysteresisRatio`（80/1.5 = 53.3%）→ `targetTokens = limit × landingPct/100` → `clearAtLeast = max(used − targetTokens, 0)`。`clearAtLeast` 同时是**准入下限**（达不到则一个字节都不改）与**回收目标**，即 cc-src 的 `trigger − keep` 形状（该字段本身是 Anthropic **服务端**参数，本地零消费，只借语义）。旧算术落点**恰为阈值线**（零下探），下一步追加 1 assistant + N tool_result 即重回越线——实测 310 次 fallback 中 164 次只释放 2 槽、49 次释放 0 槽，而每次重写都赔一整个 provider 缓存前缀。
- `compactHysteresisRatio = 1.5`（`kernel/process.go`，紧邻 `proactiveReclaimWatermarkRatio`）**必须是比例而非减法**——`threshold − 26.7` 在运维把 threshold 配到 ≤26 时变负数，落点倒挂。
- 🔴 **「回滞比」不是冷却状态机**：禁止引入任何冷却计时器 / `lastCompactStep` 进程字段。落点下探是回滞机制的**替代方案**，滞后性由结构提供（placeholder 37 字节 < `LeakedThreshold` 1000，重写过的条目永不再匹配谓词）。
- **物理可达性**：post-compact 地板受 restore 预算硬约束（files ≤50k + skills ≤25k + **plan 无任何截断**），plan 极大时落点可能物理达不到。此时事件带 `landing_reached=false` **如实上报**，不得静默失败，也不得为凑数破坏轮边界。
- **豁免仅一处**：`reclaimForResume` 不传下限——它的两个调用方在回收为空时 `finishProcess(ExitContextFull)` 杀进程，下限是一种拒绝，最后防线不能拒绝。其余全部调用点传值。

**`PruneOpts` 的两个下限判不同的量，不可互换**（Story 71.2 F4）：`MinTokens` 判 `CandidateTokens`（候选消息的**重量**），`ClearAtLeast` 判 `ReleasableTokens`（替换后**实际释放**量）。二者恒差 `11×Pruned + Σtok(ToolCallID)`（placeholder 37 字节 = 11 tokens），故传 `MinTokens: 5000` **不保证释放 5000**。两个零值都读作「无闸门」，且 `PruneOpts{}` ≡ 全零的逐字节等价性是 69.3/69.4 钉死的红线。

**compact 前置闸门**（Story 71.2 AC3）：发 LLM 请求**之前**判可压缩量，低于 `minCompactionFractionPct`(5%) 则 NOOP 直接 return——**不是 error**（五个 `Compact()` 调用方对 error 处理各异，`autoCompactIfNeeded` 的处理恰是机械兜底，闸门返 error 会让 NOOP 触发它本要避免的回收），**不发事件**（NOOP 是早退不是尝试，每步发会淹没事件流）。分子 = `reclaimable − restore 预算`（compact 会把 files/skills/plan 原样放回，这部分不因压缩而消失）；🔴 分母取**可回收池** `max(usage.Used − sysPromptTokens, 0)` 而非总用量——system prompt 永不可回收，计入会把闸门抬到大 system prompt agent 的消息历史永远够不着的高度。🔴 **闸门 fail-open**：`reclaimable <= 0` 时**照常压缩**，因为该值是跨两个来源的减法（`usage.Used` 含 ctx 自己的 system prompt，`sysPromptTokens` 读 kernel 侧 `proc` 字段），二者不一致时差值归零，fail-closed 会**永久禁用该进程的 compact**——用不可恢复的溢出换一次可恢复的 30s 超时，不对等。闸门落在 kernel 侧 `autoCompactIfNeeded` 而**不在 `Compact()` 内**，故 manual IPC + 两条 resume 路径天然绕过（等价于 qwen 的 `force` 语义），且不会误伤 `context/compact_test.go` 那 18 个 2–4 条极短消息的固件。
...
**compact 失败可观测与闩锁**（Story 71.4）：`autoCompactIfNeeded` 的失败不再静默累积（事故 308/342 次超时无人知晓），三件事：
- **started/completed 配对**（AC1）：前置闸门（71.2 AC3）**之后**、`ctxMgr.Compact()` 之前 emit 一条 `phase: "started"` 的 Compact 事件，与既有三条 completed emit（失败/成功/precompact，字段逐字保留只追加）按 `step` 关联。started 只在 `autoCompactIfNeeded` 发（reclaimLeakedIfNeeded 发 `CtxReclaim`、手动 IPC、两条 resume 路径不发）；闸门 NOOP 早退不发（否则每步一个孤儿 started）。🔴 既有 completed emit **不得删改字段**（`compact_error`/`pre_tokens`/`post_tokens`/`landing_reached` 被 71.2 测试与卷宗脚本消费）。
- **失败分类 `failure_kind`**（AC2）：失败事件追加 `failure_kind` ∈ `"timeout"`/`"cancelled"`/`"other"`（三类互斥穷尽），成功事件不加。🔴 **分类在错误产生点打哨兵，不在消费点猜**：`BuildCompactLLMCall` 的超时分支历史上**不带 `%w`**（`DeadlineExceeded` 不在 Unwrap 链上），消费点 `errors.Is(err, gocontext.DeadlineExceeded)` 会把 90% 超时静默归入 `other`——正是本 story 要修的归因失真。修法：`ErrCompactTimeout`/`ErrCompactCancelled` 哨兵 + `classifyCompactFailure(err)` 读 `errors.Is` 哨兵；cancelled 分支用多 `%w`（Go 1.20+）同时保留 `compactCtx.Err()` 在原 Unwrap 链。错误消息文案逐字保留（`compact_test.go` 断言含 `"timed out"`/`"100ms"`）。🔴 **排障提示**：`setupFallbackKernel` 注入的 `errors.New("compact: LLM call timed out after 30s")` 是**假超时**——不经 `BuildCompactLLMCall`、不带哨兵，`failure_kind` 恒为 `"other"`（这是**正确**的）；测真超时必须用 `slowMockLLMFactory`（Write 中真阻塞触发 `compactCtx` 死线）。
- **qwen 式一次闩锁**（AC3）：自动 compact 失败一次即置 `proc.CompactLatched`（`kernel/process.go`，`proc.mu` 保护），`autoCompactIfNeeded` 紧跟 `CompactionDisabled` 之后短路 `return`（不是 error/不是 selfSuspend/不是 finishProcess——AC5 best-effort 契约逐字保留）。**仅自动路径置位**（手动/resume 失败不置位）；清除点在成功分支 + kernel 导出方法 `ClearCompactLatch`（`ipc/server_observe.go` 手动 compact 成功后调用——手动成功证明「压缩现在能 work」，继续锁着是把逃生阀焊死）。🔴 **手动路径绕过是结构自带的**：`handleCompact`/两条 resume 路径直接调 `ctxMgr.Compact()`，根本不经过 `autoCompactIfNeeded`，故**不给 `Compact()` 加 `force` 参数**（与 71.2 前置闸门落点同构同理由）。🔴 **闩锁不跨 resume 存活**：`CompactLatched` 上 `vfs.ProcInfo` + IPC wire（`ProcInfoWire.compact_latched`，omitempty）但**不上 `procInfoDisk`**——resume 会重新派生 compact 超时（71.3 AC5），失败成因可能已变，带旧闩锁进新配置是把一次失败判成永久结论。
- **强制可观测三通道**（AC4，验收条件非建议）：闩锁置位同时 ①emit 独立 `CompactLatch` 事件（`step` + `failure_kind`，幂等只发一次）②`CompactLatched` 上 wire 进程级可见（`rnix ps`/IPC 消费者可读）③`log.Printf("[kernel] pid=%d step=%d compact latch set: ...")`。🔴 **不写 `proc.SuspendReason`**：`kernel/invariant.go:48-52` 禁止 `Running` 携带非空 SuspendReason（Story 44.5 AC6，`atdd_44_5_invariant_test.go:236` 守着），`TestATDD_71_4_AC4_InvariantHoldsWhenLatched` 是永久护栏。qwen 的闩锁是零信号的 private bool（telemetry 在闩锁之后根本到不了）——rnix 主动偏离参考实现。dashboard 渲染不在本 story 范围（上 wire 即满足可见性）。
