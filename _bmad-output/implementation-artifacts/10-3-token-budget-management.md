...
- [ ] Task 1: 内核层预算检查 (AC: #1, #4)
  - [ ] 1.1 `kernel/kernel.go`：`SpawnOpts` 添加 `ContextBudget int` 字段
  - [ ] 1.2 `kernel/kernel.go`：Spawn 方法中，如果 `opts.ContextBudget == 0` 且 agent 非 nil，读取 `agent.Manifest.ContextBudget` 赋值到 `proc.ContextBudget`；如果 `opts.ContextBudget > 0` 则覆盖
  - [ ] 1.3 `kernel/process.go`：Process 添加 `ContextBudget int` 字段（在 `TokensUsed` 旁）
  - [ ] 1.4 `kernel/kernel.go`：reasonStep 中 `proc.TokensUsed += resp.TokensUsed` 之后，添加预算检查——如果 `proc.ContextBudget > 0 && proc.TokensUsed >= proc.ContextBudget`，调用 `finishProcess(proc, ExitStatus{Code: 2, Reason: "budget_exceeded"})` 并 return
  - [ ] 1.5 `kernel/kernel.go`：预算超限前 emitLog `[output]` 和 emitEvent `"ReasonStep"` 记录超限详情

- [ ] Task 2: Compose 预算覆盖 (AC: #2)
  - [ ] 2.1 `compose/types.go`：`AgentSpec` 添加 `ContextBudget int \`yaml:"context_budget,omitempty"\``
  - [ ] 2.2 `compose/types.go`：`ComposeSpawnOpts` 添加 `ContextBudget int`
  - [ ] 2.3 `compose/engine.go`：spawnAgent 中将 `agentSpec.ContextBudget` 赋值到 `opts.ContextBudget`
  - [ ] 2.4 `cmd/crux/compose.go`：`ipcKernelSpawner.Spawn` 将 `ComposeSpawnOpts.ContextBudget` 传入 IPC `SpawnRequest`
  - [ ] 2.5 `ipc/protocol.go`：`SpawnRequest` 添加 `ContextBudget int \`json:"context_budget,omitempty"\``
  - [ ] 2.6 `ipc/server.go`：`handleSpawn` 解析 `ContextBudget` 并传入 `kernel.SpawnOpts`

- [ ] Task 3: ProcInfo 扩展 (AC: #5)
  - [ ] 3.1 `vfs/proc.go`：`ProcInfo` 添加 `ContextBudget int` 字段
  - [ ] 3.2 `kernel/kernel.go`：`GetProcInfo` 中将 `proc.ContextBudget` 写入 ProcInfo
  - [ ] 3.3 `ipc/protocol.go`：`ProcInfoWire` 添加 `ContextBudget int \`json:"context_budget,omitempty"\``
  - [ ] 3.4 `ipc/protocol.go`：`ProcInfoToWire` 和 `WireToProcInfo` 转换 ContextBudget
  - [ ] 3.5 `vfs/proc.go`：`statusJSON` 添加 `ContextBudget` 字段

- [ ] Task 4: crux top 预算警告显示 (AC: #3, #4)
  - [ ] 4.1 `cmd/crux/top.go`：进程列表渲染中，如果 `proc.ContextBudget > 0`，TOKENS 列改为 `已用/预算` 格式
  - [ ] 4.2 `cmd/crux/top.go`：如果 `proc.ContextBudget > 0 && proc.TokensUsed >= proc.ContextBudget * 90 / 100`，用 `WarningStyle` 渲染 TOKENS 列
  - [ ] 4.3 `cmd/crux/top.go`：topSummaryLine 中总 tokens 显示不变（仍为总消耗纯数字）
  - [ ] 4.4 `cmd/crux/top.go`：topDetailView 中增加 Budget 行（仅在 budget > 0 时显示）

- [ ] Task 5: 测试 (AC: all)
  - [ ] 5.1 `kernel/budget_test.go`：预算检查单元测试——mock LLM 返回固定 token，验证达到预算时进程转 Zombie + exit reason = "budget_exceeded"
  - [ ] 5.2 `kernel/budget_test.go`：无预算时（budget=0）不触发终止
  - [ ] 5.3 `kernel/budget_test.go`：Spawn 传入 ContextBudget 覆盖 agent 默认值
  - [ ] 5.4 `compose/engine_test.go`：验证 AgentSpec.ContextBudget 传递到 ComposeSpawnOpts
  - [ ] 5.5 `cmd/crux/top_test.go`：验证预算警告格式（`已用/预算`）和无预算纯数字格式
  - [ ] 5.6 `ipc/protocol_test.go`：ProcInfoWire 往返转换包含 ContextBudget
  - [ ] 5.7 在 `cmd/crux/main_test.go` 中确认无命令注册回归
