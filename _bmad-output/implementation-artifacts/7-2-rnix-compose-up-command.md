...
1. **compose up 子命令注册** — Given `cmd/rnix/compose.go` 中 compose up 子命令已注册，When 执行 `rnix compose up`，Then 读取当前目录的 `rnix-compose.yaml`，And 按 DAG 顺序 Spawn 所有智能体，And 实时输出每个智能体的启动和完成状态

2. **自定义文件** — Given 指定自定义文件，When 执行 `rnix compose up -f my-workflow.yaml`，Then 使用指定文件而非默认文件
...
- [x] Task 1: 创建 compose 子命令和 CLI 注册 (AC: #1, #2)
  - [x] 1.1 创建 `cmd/rnix/compose.go`，注册 `compose` 父命令和 `compose up` 子命令
  - [x] 1.2 添加 `-f/--file` flag（默认 `rnix-compose.yaml`）
  - [x] 1.3 在 `cmd/rnix/main.go` 的 `init()` 中注册 `composeCmd` 到 `rootCmd`
  - [x] 1.4 支持全局 flags（`--json`、`--verbose`、`--quiet`）

- [x] Task 2: 实现 KernelSpawner 适配器 (AC: #1)
  - [x] 2.1 在 `cmd/rnix/compose.go` 中实现 `ipcKernelSpawner` 结构体，适配 `compose.KernelSpawner` 接口
...
- [x] Task 7: 单元测试和集成测试 (AC: #1-4)
  - [x] 7.1 `cmd/rnix/compose_test.go` — TestComposeUp_DefaultFile：默认读取 rnix-compose.yaml
...
**IPC 适配器模式**：`rnix compose up` 作为 CLI 客户端命令，通过 IPC 与 daemon 通信。Story 7.1 的 `compose.Engine` 在**daemon 侧**运行，CLI 侧负责解析 YAML、创建引擎、通过 IPC 调度。
...
var composeUpCmd = &cobra.Command{
    ...
    Example: `  rnix compose up                      # 使用当前目录的 rnix-compose.yaml
  rnix compose up -f my-workflow.yaml   # 使用指定文件
  rnix compose up --json                # JSON 输出模式`,
    RunE: runComposeUp,
}
...
**不修改 compose 包**：Story 7.2 仅在 `cmd/rnix/` 层添加 CLI 命令和 IPC 适配器，`compose/` 包完全不变。
...
cmd/rnix/compose_test.go       # compose up 测试
...
**修改文件：**
```
cmd/rnix/main.go              # init() 中添加 composeCmd 注册（约 1 行）
```
...
cmd/rnix/compose.go → agents/         （AgentLoader）
cmd/rnix/compose.go → skills/         （SkillLoader）
cmd/rnix/compose.go → internal/ui/    （编排汇总 UI）
...
- [Source: _bmad-output/planning-artifacts/prd/functional-requirements.md#FR48] — rnix compose up 一键启动
...
- [Source: _bmad-output/implementation-artifacts/7-1-rnix-compose-yaml-parsing-and-dag-scheduling-engine.md] — Story 7.1 实现，compose 包设计和接口
...
- [Source: cmd/rnix/main.go] — 现有 CLI 结构、init() 命令注册模式、信号处理模式
...
- compose 包完全未修改，仅在 cmd/rnix/ 层添加 CLI 和适配器
...
- 2026-03-01: 实现 Story 7.2 rnix compose up 命令 — CLI 注册、IPC KernelSpawner 适配器、编排汇总 UI、失败传播、信号处理
- 2026-03-01: 代码审查修复 — 补全 token 消耗显示（AC #4）、移除死代码（killClient/RenderComposeProgressPID）、sync.Mutex+map 替换为 xsync.SyncMap

### File List

- `cmd/rnix/compose.go` — 新增：compose 命令注册 + compose up 实现 + IPC 适配器（ipcKernelSpawner）
- `cmd/rnix/compose_test.go` — 修改：修复 ATDD 测试中 mock spawner 的 failAgents→failIntents、ui 函数调用前缀、适配 SyncMap 重构
- `cmd/rnix/main.go` — 修改：init() 中添加 `rootCmd.AddCommand(composeCmd)`（1 行）
