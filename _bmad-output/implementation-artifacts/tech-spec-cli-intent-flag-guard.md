...
files_to_modify: ['cmd/rnix/main.go', 'cmd/rnix/main_test.go']
...
当前 `rnix top` 这样的裸参数被 `rootCmd`（`Args: cobra.ArbitraryArgs`）直接当作 intent 字符串送给 agent，导致：
...
| File | Purpose |
| ---- | ------- |
| `cmd/rnix/main.go` — 全局 flag 变量块 | `flagJSON`...`flagAgent` 定义区域 |
| `cmd/rnix/main.go` — `rootCmd` 变量 | rootCmd 定义（Use/Args/RunE） |
| `cmd/rnix/main.go` — `init()` 函数 | flag 注册 + 子命令注册 |
| `cmd/rnix/main.go` — `runRoot()` 函数 | 读取 args 构建 intent 并 spawn |
| `cmd/rnix/main.go` — `runDaemon()` 函数 | 含引用旧 intent 用法的硬编码错误消息 |
| `cmd/rnix/main_test.go` — `TestHelp_*` | Help 输出测试 |
| `cmd/rnix/main_test.go` — `TestVersion_*` | flag save/restore 模式参考 |
| `cmd/rnix/main_test.go` — `setupTestIPCServer` | IPC 测试基础设施 |
| `cmd/rnix/main_test.go` — `TestRunKill_PIDNotFound_ViaIPC` | SocketPathOverride 使用示例 |

**注意**：以上引用的是函数/变量名而非行号。随着 Task 顺序执行，行号会偏移，请以函数/变量名定位。

### Technical Decisions

- **Breaking change**：`rnix "意图"` 不再工作，必须用 `rnix -i "意图"`
...
- **`-i` 使用 `Flags()` 而非 `PersistentFlags()`**：intent flag 仅在 root 命令有意义，不应向子命令传播。`rnix -i "intent" version` 时 Cobra 路由到 version 子命令，由于 `-i` 是 root 的 local flag 而非 persistent flag，Cobra 会报 `unknown flag` 错误。这与现有 `--model`/`--agent` 行为一致，是预期的
...
- [x] Task 4: 实现 `rejectPositionalArgs()` 自定义 Args 验证器
  ...
  - Action: 添加 `func rejectPositionalArgs(cmd *cobra.Command, args []string) error`。如果 `len(args) == 0` 返回 nil。否则将所有 args 用空格 join 为展示文本 `display := strings.Join(args, " ")`，调用 `suggestCommand(cmd, args[0])`（模糊匹配只用第一个词），构建错误信息。**错误消息规范格式**：
    - 有建议时：`unknown command "%s", did you mean "%s"?\n\n  Usage: rnix -i <intent>\n  Run 'rnix --help' for available commands.`（第一个 `%s` = display，第二个 `%s` = suggestion）
    - 无建议时：`unknown command "%s"\n\n  Usage: rnix -i <intent>\n  Run 'rnix --help' for available commands.`（`%s` = display）
...
- [x] Task 5: 更新 `rootCmd` 定义
  - File: `cmd/rnix/main.go` — `rootCmd` 变量 + `init()` 函数
  - Action: 修改以下字段：
    - `Use`: `"rnix [command]"`（标准 cobra 模式，`-i` 在 Long/Example 中说明）
...
- [x] Task 6: 修改 `runRoot` 读取 intent 来源
  - File: `cmd/rnix/main.go` — `runRoot()` 函数
...
- [x] Task 7: 更新 `runDaemon` 错误消息
  - File: `cmd/rnix/main.go` — `runDaemon()` 函数
  - Action: 将 `"daemon command is for internal use only; use 'rnix \"intent\"' to run agents"` 改为 `"daemon command is for internal use only; use 'rnix -i \"intent\"' to run agents"`
  - Notes: 与 breaking change 保持一致

- [x] Task 8: 更新现有测试
  - File: `cmd/rnix/main_test.go`
  - Action:
    - `TestHelp_ContainsExample`：将断言更新为同时检查 `strings.Contains(output, "rnix -i")` 和 `strings.Contains(output, "分析 ./README.md")`，确保新格式和内容都被验证
...
- [x] Task 10: 新增裸参数拒绝测试
  ...
  - Action: 添加以下测试（**直接调用 `rejectPositionalArgs(rootCmd, args)` 并检查 `error.Error()` 子串**）：
    ...
    - `TestRejectPositionalArgs_UnknownWord`: `rejectPositionalArgs(rootCmd, []string{"top"})` → `err.Error()` 包含 `unknown command "top"` 和 `rnix -i <intent>`，且**不包含** `did you mean`
    ...
    - `TestRejectPositionalArgs_IntentPlusBareArgs`: `rejectPositionalArgs(rootCmd, []string{"bar"})` → 返回 error（即使 `flagIntent` 已设置也无影响，因为 `rejectPositionalArgs` 不检查 flag，它只看 positional args）。实际场景：`rnix -i "foo" bar` 时 Cobra 先解析 flag 设置 `flagIntent="foo"`，再将 `["bar"]` 传给 Args 验证器，验证失败 → 报错，不启动 agent
...
- [x] AC 1: Given 用户执行 `rnix -i "分析代码"`, when CLI 处理参数, then intent 被正确传递给 `ipc.SpawnRequest.Intent` 并启动 agent
- [x] AC 2: Given 用户执行 `rnix`（无参数无 flag）, when CLI 处理参数, then 显示 help 信息
- [x] AC 3: Given 用户执行 `rnix top`（单个裸参数，无 `-i`）, when CLI 处理参数, then 输出错误含 `unknown command "top"` 和 `rnix -i <intent>` 且不启动 agent
- [x] AC 4: Given 用户执行 `rnix ver`（裸参数，近似子命令）, when CLI 处理参数, then 输出含 `unknown command "ver", did you mean "version"?` 且不启动 agent
- [x] AC 5: Given 用户执行 `rnix --json top`（带 `--json` 的裸参数）, when CLI 处理参数, then 输出纯文本错误（不含 JSON 格式），不受 `--json` 影响
- [x] AC 6: Given 用户执行 `rnix deamon`（hidden 子命令拼写错误）, when CLI 处理参数, then 输出含 `did you mean "daemon"?`
- [x] AC 7: Given 用户执行 `rnix -i "分析代码" --json --model opus`, when CLI 处理参数, then 所有 flag 正常解析，intent + model + json 均正确传递
- [x] AC 8: Given 用户执行 `rnix --help`, when CLI 处理参数, then help 文本包含 `-i, --intent` flag 说明和新格式的 example（含 `rnix -i`）
- [x] AC 9: Given 用户执行 `rnix foo bar baz`（多个裸参数）, when CLI 处理参数, then 输出含 `unknown command "foo bar baz"` 且不启动 agent
- [x] AC 10: Given 用户执行 `rnix is`（≤ 3 字符短输入）, when CLI 处理参数, then 输出 `unknown command "is"` 且不建议 `"ps"`（短输入不做 Levenshtein）
- [x] AC 11: Given 用户执行 `rnix ki`（≤ 3 字符但是合法前缀）, when CLI 处理参数, then 输出含 `did you mean "kill"?`（前缀匹配不受短输入保护限制）
- [x] AC 12: Given 用户执行 `rnix kil`（len=3 前缀匹配边界）, when CLI 处理参数, then 输出含 `did you mean "kill"?`（`strings.HasPrefix("kill", "kil")` 为 true）
- [x] AC 13: Given 用户执行 `rnix -i "foo" bar`（同时有 intent flag 和裸参数）, when CLI 处理参数, then 输出 `unknown command "bar"` 错误且不启动 agent（Args 验证优先于 RunE）
...
**单元测试（全部在 `cmd/rnix/main_test.go`）：**
...
**手动验证：**
- `rnix -i "hello"` → 正常启动 agent
- `rnix top` → 纯文本错误，无 agent 启动
- `rnix ver` → "did you mean version?"
- `rnix ki` → "did you mean kill?"（前缀匹配）
- `rnix foo bar` → `unknown command "foo bar"`
- `rnix is` → `unknown command "is"`（无建议）
- `rnix --json top` → 纯文本错误（非 JSON）
- `rnix` → 显示 help
- `rnix --help` → 显示含 `-i` flag 的帮助

### Notes

- 这是一个 **breaking change**：所有现有的 `rnix "意图"` 用法需迁移为 `rnix -i "意图"`
...
- `rnix -i "intent" version` 时 Cobra 路由到 version 子命令，由于 `-i` 是 root 的 local flag，Cobra 报 `unknown flag` 错误。这与 `--model`/`--agent` 行为一致，是预期的
- `rnix -i "intent" -- extra` 中 `--` 后的 tokens 作为 positional args 被 `rejectPositionalArgs` 拦截。这是预期行为：`--` 传递在此 CLI 中无意义
- `rnix -i "foo" bar` 中 Cobra 先解析 `-i "foo"` 设置 `flagIntent`，再将 `["bar"]` 传给 `rejectPositionalArgs`，验证失败报错，`RunE` 不会被调用，agent 不会启动
