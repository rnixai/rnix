...
**Given** 进程被挂起
**When** 用户审查后
**Then** 可通过 `crux immune resume <pid>` 恢复执行或 kill 终止

## Story 22.3: 安全状态管理

As a 平台构建者,
I want 通过 `crux immune status` 查看完整的安全监控状态,
So that 我可以全面了解系统的安全态势。

**Acceptance Criteria:**

**Given** Immune Daemon 运行中
**When** 用户执行 `crux immune status`
...
As a 平台构建者,
I want 系统自动识别高频协作路径，并通过 `crux topology` 查看协作拓扑图,
