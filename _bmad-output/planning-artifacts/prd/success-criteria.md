...
| 指标 | 衡量方式 | 目标 |
|------|---------|------|
| 调试效率 | 定位多智能体 bug 的时间 | 从"天级"降至 ≤10 分钟（单智能体 bug 的平均定位时间） |
| 能力复用率 | 单个 Skill 被引用的项目数 | ≥ 3 个项目 |
| 上手门槛 | 安装到跑通第一个 demo | ≤ 15 分钟 |
| 顿悟时刻 | `strace` 首次定位到真实问题 | 用户通过 strace 在 ≤5 个事件内定位到产生错误的 syscall 调用记录 |

**用户 B（应用开发者）：**

| 指标 | 衡量方式 | 目标 |
|------|---------|------|
| 构建效率 | 完成多智能体工作流的代码量 | 比现有框架减少 90%（以等价 LangChain/CrewAI 实现的代码行数为基线） |
| Skill 生态 | skillpkg 可用 Skill 数量 | 发布 6 个月内社区贡献 ≥10 个可复用 Skill |
...
| 12 个月 | GitHub Stars ≥ 500，支撑指标（demo 成功率、Skill 数量、Contributor 数量、外部引用）同步跟踪 |
...
| 容错恢复 | Supervisor 重启后任务在 SLA 内完成，无用户可见错误输出 |
