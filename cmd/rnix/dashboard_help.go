...
func (m dashboardModel) renderHelpOverlay() string {
	...
	left.WriteString(entry("s", "Toggle step / syscall") + "\n")
	left.WriteString(entry("h/l", "Scroll timeline") + "\n")
	left.WriteString(entry("v", "Expand step detail") + "\n")
	left.WriteString(entry("p", "Prompt viewer") + "\n")
	left.WriteString(entry("+/-", "Zoom time bar") + "\n")
	left.WriteString("\n")
	left.WriteString(groupTitle.Render("Filter Mode") + dimStyle.Render(" (press f)") + "\n")
	left.WriteString(entry("l", "Toggle LLM") + "\n")
	left.WriteString(entry("t", "Toggle Tool") + "\n")
	left.WriteString(entry("i", "Toggle IPC") + "\n")
	left.WriteString(entry("v", "Toggle VFS") + "\n")
	left.WriteString(entry("a", "Enable all") + "\n")
