...
func TestATDD_28_2_AC6_LegacyPIDPath_GetStepDetail_ReturnsNotFound(t *testing.T) {
	...
	pid := types.PID(8888)
	stepsDir := filepath.Join(projBase, "steps", fmt.Sprintf("%d", pid))
	if err := os.MkdirAll(stepsDir, 0o755); err != nil {
		t.Fatalf("AC-6: mkdir: %v", err)
	}

	writeTestSteps(t, projBase, pid, []types.StepRecord{
		testStepRecord(1),
	})
...
func writeTestStepsUUID(t *testing.T, projBaseDir string, procUUID string, records []types.StepRecord) {
	t.Helper()
