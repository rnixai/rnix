...
func (k *KernelImpl) GetProcInfo(pid types.PID) (*vfs.ProcInfo, error) {
	...
	info := &vfs.ProcInfo{
		...
		// Story 74.2 — four-way cumulatives mirror the authoritative values
		// as-is. Deliberately NOT synthesized with mid-stream deltas: no
		// decomposition exists (AddStreamUsage only accumulates merged
		// StreamTokensUsed), so they honestly lag the preview TokensUsed
		// during a long CLI step (documented degradation, not a bug).
		CumInputTokens:              proc.CumInputTokens,
		CumCachedInputTokens:        proc.CumCachedInputTokens,
		CumCacheCreationInputTokens: proc.CumCacheCreationInputTokens,
		CumOutputTokens:             proc.CumOutputTokens,
...
func (k *KernelImpl) ListProcs() []vfs.ProcInfo {
	...
	k.procTable.Range(func(_ types.PID, proc *Process) bool {
		...
		infos = append(infos, vfs.ProcInfo{
			...
			ToolCallCount:   proc.ToolCallCount,
			LastInputTokens: proc.LastInputTokens,
			// Story 74.2 — see GetProcInfo: cumulatives as-is, no synthesis.
			CumInputTokens:              proc.CumInputTokens,
			CumCachedInputTokens:        proc.CumCachedInputTokens,
			CumCacheCreationInputTokens: proc.CumCacheCreationInputTokens,
			CumOutputTokens:             proc.CumOutputTokens,
